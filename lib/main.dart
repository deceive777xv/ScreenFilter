...
class _FilterOverlayPageState extends State<FilterOverlayPage> {
  ...
  late final bool _nativeRestorePendingOnStartup;
  ...
  void initState() {
    ...
    final hasLastNativeFilterState =
        _settings.getLastNativeFilterState() != null;
    _nativeRestorePendingOnStartup = shouldPrimeNativeRestoreOnStartup(
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await _prepareStartupNativeRestoreWindow();
      final nativeRestoreSucceeded = await _restoreLastNativeFilterOnStartup();
      await _finishStartupNativeRestoreAttempt(nativeRestoreSucceeded);
    });
  ...
  Future<void> _prepareStartupNativeRestoreWindow() async {
    if (!_nativeRestorePendingOnStartup || !mounted) return;
  ...
  void _disableStartupBaseShaderSuppression() {
    if (!_suppressBaseShaderForStartupNativeRestore) return;
  ...
  Future<bool> _restoreLastNativeFilterOnStartup() async {
    if (!mounted || _shaderFilterService.mode != FilterApplyMode.none) {
      return false;
    }
    ...
    final restoreScreenSize = await _waitForStableFullscreenSurface();
    if (!mounted || restoreScreenSize == Size.zero) return false;
    ...
    await _clearFlutterSurfaceBeforeNativeRestore();
    ...
    _shaderFilterService.updateScreenSize(restoreScreenSize);
    _shaderFilterService.updateDevicePixelRatio(dpr);
    _shaderFilterService.updateAccentColor(snapshot.accentColor);
    _shaderFilterService.updateFilterVisuals(
      opacity: _alpha,
      brightness: _brightness,
  ...
  Future<Size> _waitForStableFullscreenSurface() async {
    await windowManager.setFullScreen(true);
    Size? previousSize;
    var stableFrames = 0;
    ...
    while (mounted && DateTime.now().isBefore(deadline)) {
      ...
      final size = _currentFlutterViewLogicalSize();
      if (isFullscreen && _isUsableSurfaceSize(size)) {
        final previous = previousSize;
        stableFrames = previous != null && _sizesClose(size, previous)
            ? stableFrames + 1
            : 0;
        if (stableFrames >= 1) return size;
      }
  ...
  Size _currentFlutterViewLogicalSize() {
  ...
  bool _isUsableSurfaceSize(Size size) => size.width > 0 && size.height > 0;

  bool _sizesClose(Size a, Size b) {
    const tolerance = 1.0;
    return (a.width - b.width).abs() <= tolerance &&
        (a.height - b.height).abs() <= tolerance;
  }

  Future<void> _clearFlutterSurfaceBeforeNativeRestore() async {
    setState(() => _clearSurfaceBeforeNativeRestore = true);
    await WidgetsBinding.instance.endOfFrame;
    if (!mounted) return;
    setState(() => _clearSurfaceBeforeNativeRestore = false);
    await WidgetsBinding.instance.endOfFrame;
  }
  ...
  void _applyBasicFilterValues({
    ...
    setState(() {
      _disableStartupBaseShaderSuppression();
  ...
  Widget _buildShaderFilter() {
    ...
    if (!shouldPaintBaseShader(
      ...
      forceClear: _clearSurfaceBeforeNativeRestore,
      suppressForNativeRestore: _suppressBaseShaderForStartupNativeRestore,
    )) {
      if (shouldClearBaseShaderSurface(
        shaderLoaded: _shader != null,
        sandboxActive: sandboxActive,
        baseFilterEnabled: _baseFilterEnabled,
        forceClear: _clearSurfaceBeforeNativeRestore,
        ...
        return const CustomPaint(
          size: Size.infinite,
          painter: TransparentSurfacePainter(),
        );
      }
      return const SizedBox();
    }
...
class TransparentSurfacePainter extends CustomPainter {
  const TransparentSurfacePainter();
  ...
  @override
  bool shouldRepaint(covariant TransparentSurfacePainter oldDelegate) => false;
}
